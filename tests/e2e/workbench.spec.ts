...
test('persists a solved dynamic recipe and keeps its gearset when the job level changes', async ({ page }) => {
  ...
  await expect(page.getByLabel('職業等級', { exact: true })).toHaveCount(0)
