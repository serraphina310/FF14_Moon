...
| Profile-sensitive solution state | Domain and Chrome tests make solutions stale across effective profile edits and active-profile switches, show `裝備有更新` when the active profile timestamp is later than the solve, glow the solve button for stale adopted solutions, and still make option and version changes stale. Saved profile snapshots remain immutable. |
